...
- 395e2a1: Carry the telemetry dimensions `seamless-auth-api` now records on every auth event.
...
- 7390df6: Default `authenticator_policy.syncedPasskeys` to `allow`.
...
- a75751e: Let a deployment ask authenticators to identify themselves.
...
- ab19e4a: Let a deployment state whether the authenticator must verify the human.
...
- e89de6c: Record which authenticator a credential came from.
...
- 9a836dd: Correlate audit events to the session they happened in.

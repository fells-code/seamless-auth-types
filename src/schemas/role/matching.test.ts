...
import { ROLE_NAME_PATTERN, hasScopedRole, roleGrantsAccess } from './matching.js';

describe('matching module', () => {
  it('stays free of Zod so consumers can import it without the dependency', () => {
    const source = readFileSync(new URL('./matching.ts', import.meta.url), 'utf8');
    expect(source).not.toMatch(/from ['"]zod/);
  });
});

describe('ROLE_NAME_PATTERN', () => {
  it.each(['admin', 'billing:invoices:read', 'billing-team:read'])('accepts %j', (value) => {
    expect(ROLE_NAME_PATTERN.test(value)).toBe(true);
  });

  it.each(['bad role', 'bad_role', 'bad/role', 'bad\\role', '', 'a'.repeat(81)])(
    'rejects %j',
    (value) => {
      expect(ROLE_NAME_PATTERN.test(value)).toBe(false);
    },
  );
});
